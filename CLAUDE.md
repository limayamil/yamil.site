...
- [CompanyLogo.astro](src/components/CompanyLogo.astro) + [src/lib/company-logos.ts](src/lib/company-logos.ts) — the employer wordmarks in the track record, and the only marks whose shape is not ours. Same build-time contract as `toolIcons`: an unknown `company` throws. Sized by *area*, not by a shared height — `width`/`height: auto` plus the mark's own `aspect-ratio` (written inline from its `viewBox`) makes `.track-company-logo`'s `max-width`/`max-height` behave like `object-fit: contain`, which is what lets an 8.9:1 wordmark and a 1.2:1 one sit in the same column. Unlike the other three these are **not** `aria-hidden`: they carry `role="img"` and the company name as `aria-label`, because here the logo replaced the text instead of decorating it. See [design.md §9](design.md).
